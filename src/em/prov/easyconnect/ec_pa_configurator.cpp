...
bool ec_pa_configurator_t::handle_presence_announcement(ec_frame_t *frame, size_t len, uint8_t src_mac[ETHER_ADDR_LEN])
{
    if (frame == NULL || src_mac == NULL) {
        em_printfout("Invalid NULL parameter in handle_presence_announcement");
        return false;
    }
...
bool ec_pa_configurator_t::handle_recfg_announcement(ec_frame_t *frame, size_t len, uint8_t sa[ETH_ALEN], uint8_t src_al_mac[ETH_ALEN])
{
    ...
    if (frame == NULL || sa == NULL) {
        em_printfout("Invalid NULL parameter in handle_recfg_announcement");
        return false;
    }
...
bool ec_pa_configurator_t::handle_auth_response(ec_frame_t *frame, size_t len, uint8_t src_mac[ETHER_ADDR_LEN], uint8_t src_al_mac[ETH_ALEN])
{
    (void)src_al_mac; // Unused parameter in proxy agent
    if (frame == NULL || src_mac == NULL) {
        em_printfout("Invalid NULL parameter in handle_auth_response");
        return false;
    }
...
{
    if (buff == NULL || sa == NULL) {
        em_printfout("Invalid NULL parameter in handle_cfg_request");
        return false;
    }
...
{
    if (frame == NULL || sa == NULL) {
        em_printfout("Invalid NULL parameter in handle_cfg_result");
        return false;
    }
...
{
    if (frame == NULL || sa == NULL) {
        em_printfout("Invalid NULL parameter in handle_connection_status_result");
        return false;
    }
...
{
    if (sa == NULL) {
        em_printfout("Invalid NULL source MAC in handle_gas_comeback_request");
        return false;
    }
