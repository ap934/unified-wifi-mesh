...
{
	if (!subdoc || !str || strlen(str) == 0) return -1;
	em_long_string_t key;
...
{
    if (!subdoc || !key || strlen(key) == 0) return -1;
...
int dm_easy_mesh_t::encode_config_op_class_array(cJSON *arr_obj, em_op_class_type_t type, unsigned char *mac)
{
	...
	if (!cJSON_IsArray(arr_obj)) return -1;
	unsigned int i;
	cJSON *op_obj;
	mac_addr_str_t	mac_str;
...
{
    if (index >= m_num_opclass) return nullptr;
...
{
    if (!src_mac_addr || !agent_al_mac || !autoconfig_renew_json) throw std::invalid_argument("invalid argument");
...
{
    if (!src_mac_addr || !agent_al_mac || !ap_query_json || msg_id < 0) throw std::invalid_argument("invalid argument");
...
{
    if (!src_mac_addr || !agent_al_mac || !ap_query_json || !mac || msg_id < 0) throw std::invalid_argument("invalid argument");
    cJSON *root, *query_info, *device_list;
    root = cJSON_CreateObject();
    query_info = cJSON_CreateObject();
    device_list = cJSON_CreateArray();
...
{
    if (!sta_info) throw std::invalid_argument("null sta_info");
    hash_map_t *map;
    const char	*map_str;
    mac_addr_str_t radio_str, bss_str, sta_str;
...
{
    if (!scan_result) throw std::invalid_argument("null scan_result");
...
{
    if (type == db_cfg_type_none) return;
...
{
    if (cfg_type == db_cfg_type_none) return nullptr;
	unsigned int num = 0;
